...
var swiper = () => {
   // var servicePackages = new Swiper('.service-packages__list', {
   //    init: false,
   ...
   var _targetAdsFunc = () => {
      var breakpoint = window.matchMedia('all and (min-width: 768px) and (max-width: 1199px)');
      
      var targetAds;

      var breakpointChecker = () => {
         if (breakpoint.matches === true) {
            _targetAdsFuncCreate();
            return;
         } else if (targetAds !== undefined) targetAds.destroy(true, true);
      };

      var _targetAdsFuncCreate = () => {
         targetAds = new Swiper('.targeting-ads__list', {
   ...
   if ($.querySelector('.targeting-ads__list')) _targetAdsFunc();
};